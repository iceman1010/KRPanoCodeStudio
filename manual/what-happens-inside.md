...
| `plan_files` | The AI wants to pre-read a set of files before editing and is asking you to confirm. Carries the file list + a short reason. Status can be `ask` (waiting for you), `yes` (you approved), `no` (you declined), or `auto-approved` (the Settings "Auto-approve file scope plan" option is on — the plan was recorded but the banner was skipped). | **Plan files panel** appears with the proposed files and reason; you Approve or Decline. Status turns amber while waiting. With `auto-approved`, the banner is skipped and the edit proceeds; the conversation log still records the auto-approved plan. |
